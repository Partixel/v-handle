function RenderCall(Frame, Table)
	...
	for _, a in pairs(vh.Modules) do
		...
		for i, v in pairs(a.Commands) do
			...
			Table[i]:SetPos(Frame:GetWide() * 0.2, 10 + (35 * CurrentCommands))
			Table[i]:SetSize(Frame:GetWide() * 0.6, 30)
			Table[i]:SetText("")
			Table[i].Paint = function()
				surface.SetDrawColor(_V.MenuLib.GetSettings().Colors.QuadButton)
				surface.DrawRect(0, 0, Table[i]:GetWide(), Table[i]:GetTall())

...
V-LogLib.LogLocation = V-LogLib.LogLocation or "vh/Logs" -- The directory the file is located in
V-LogLib.LogFile = V-LogLib.LogFile or "log" -- The name of the file - Date + .txt are appended on after this
V-LogLib.MessageLevel = V-LogLib.MessageLevel or 4 -- Any log type lower or equal to this will be printed to console
V-LogLib.LogLevel = V-LogLib.LogLevel or 1 -- Any log type between or equal to this and the MessageLevel will be written to file

file.CreateDir(V-LogLib.LogLocation) -- Make sure the directories exist and initializes LogLocation
...
function V-LogLib.Log(Message, Type)
	...
	elseif Message != nil then
		...
		local MessageLevel = V-LogLib.MessageLevel
		...
		if V-LogLib.LogLevel <= Type and Type <= MessageLevel then
			-- Checks if the message should be logged to file
			local CurrentFile = tostring(os.date("%d.%m.%y-" .. V-LogLib.LogFile .. ".txt"))
			...
			file.Append(V-LogLib.LogLocation .. "/" .. CurrentFile, Message .. "\n")
